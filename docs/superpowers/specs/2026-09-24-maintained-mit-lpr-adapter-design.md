...
The existing v1 recognizer contract is fixed at grayscale `[B,1,64,160]`, 80 CTC steps and blank index 0. The author's models use a plate-centric color ROI, CPM at 100×100, a 94×48 perspective crop, and a 37-entry class list including `I` and `O`; its final `-` entry is used as the blank class, not as a printed separator. Do **not** mislabel these ONNX models as a v1 `recognizer.onnx` or feed their logits to `decode_constrained_ctc_v1`.
...
The Web comparison path uses a fixed allowlisted option for this adapter alongside the existing `active-v1` and `candidate-v2-retest` choices; request data cannot name an arbitrary filesystem path. It reports which detector and recognizer produced every result and keeps rejected candidates visible in diagnostics. The existing activation route must not copy this unlike-schema bundle onto `active-v1`; activation/default changes require their own explicit gate after evidence is reviewed. The source-only 1788 API scaffold is not silently upgraded to a working service by this change; a later release integration must include the inference path, notices, and model files together.
