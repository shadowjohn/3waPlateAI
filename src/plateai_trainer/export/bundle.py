...
def export_crop_bundle(request: ExportRequest, session_factory: Callable[[Path], OrtSession] = create_cpu_session) -> Path:
    ...
    if checkpoint.get("class_count") != class_count or checkpoint.get("charset_sha256") != charset.sha256 or checkpoint.get("rules_sha256") != rules_hash or checkpoint.get("preprocess") != asdict(V1_PREPROCESS): raise ValueError("checkpoint is not compatible with the v1 export contract")
