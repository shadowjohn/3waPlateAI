...
def export_crop_bundle(request: ExportRequest, session_factory: Callable[[Path], OrtSession] = create_cpu_session) -> Path:
    ...
    charset_path, rules_path = _paths(); charset = load_character_set(charset_path); rules_hash = _sha(rules_path)
    checkpoint = torch.load(request.checkpoint, map_location="cpu", weights_only=True)
    if checkpoint.get("class_count") != 34 or checkpoint.get("charset_sha256") != charset.sha256 or checkpoint.get("rules_sha256") != rules_hash or checkpoint.get("preprocess") != asdict(V1_PREPROCESS): raise ValueError("checkpoint is not compatible with the v1 export contract")
    model = PlateCTCNet(); model.load_state_dict(checkpoint["model_state_dict"]); model.eval(); torch.set_num_threads(1)
    ...
    try:
        ...
        if len(inputs)!=1 or len(outputs)!=1 or inputs[0].name!="input" or outputs[0].name!="logits" or inputs[0].type!="tensor(float)" or outputs[0].type!="tensor(float)" or list(inputs[0].shape)[1:]!=[1,64,160] or list(outputs[0].shape)[1:]!=[80,34]: raise ExportParityError("ONNX IO contract differs")
        _parity(model, session, CTCCodec.from_charset(charset))
        shutil.copyfile(charset_path, staging / "charset.txt"); shutil.copyfile(rules_path, staging / "plate_rules.json"); shutil.copyfile(request.report, staging / "report.json")
        manifest={"schema_version":1,"contract_version":"1.0","model_id":"twplate-v1-recognizer","version":"1.0.0","created_at":"2026-09-21T00:00:00Z","capabilities":["crop-recognition"],"plate_size":[380,160],"charset":{"file":"charset.txt","sha256":_sha(staging/"charset.txt"),"visible_symbols":33},"rules":{"file":"plate_rules.json","sha256":_sha(staging/"plate_rules.json")},"components":{"recognizer":{"file":"recognizer.onnx","format":"onnx","sha256":_sha(onnx_path),"inputs":[{"name":"input","dtype":"float32","shape":["batch",1,64,160]}],"outputs":[{"name":"logits","dtype":"float32","shape":["batch",80,34]}],"batch":{"mode":"dynamic","min":1,"opt":8,"max":32}}},"decoder":{"type":"ctc","blank_index":0,"class_count":34,"index_mapping":"charset-order-skipping-blank","collapse_repeats":True},"preprocess":{"source_size_wh":[380,160],"color_space":"grayscale","layout":"NCHW","input_size_hw":[64,160],"grayscale":{"reference":"pillow-image-convert-l","library_version":"12.3.0"},"resize":{"mode":"letterbox","reference":"pillow-image-resize","interpolation":"Resampling.BILINEAR","library_version":"12.3.0","resized_size_hw":[64,152],"padding_ltrb":[4,0,4,0],"padding_raw_value":255},"normalization":{"type":"divide","divisor":255.0,"dtype":"float32"}},"provenance":{"training_data":"synthetic","license_reviewed":True,"training_report":{"file":"report.json","sha256":_sha(staging/"report.json")}}}
