...
- Full local verification: `.venv\Scripts\python.exe -m pytest -q` completed with `86 passed in 26.80s`; source distribution and wheel builds succeeded and included `plateai_reader`; `.venv\Scripts\python.exe -m pip check` reported no broken requirements.
